...
def hashDictForPunctions() -> typing.Dict[str, str]:
    """
...
class Preprocessor:
    ...
    def __init__(self, QuestionNumber: int, sentenceInputPath: typing.Union[str, os.PathLike],
        ...
        try:
            ...
            self.__SentenceInputPath = self.__validate_file_path(sentenceInputPath, "Sentence Input Path")
            if peopleInputPath is not None:
                self.__PeopleInputPath = self.__validate_file_path(peopleInputPath, "People Input Path")
            self.__RemoveInputPath = self.__validate_file_path(removeInputPath, "Remove Input Path")
        ...
        self.__Removes = {key: True for key in processedWords}  # hash dictionary time
        # complicity O(1) when when check member ship

        # Process __sentences after __Removes has been initialized
        self.__Sentences = [self.__removeUnwantedWords(self.process_sentence(line)).split() for line in
                            self.__read_csv_to_list(self.__SentenceInputPath) if
                            len(self.__removeUnwantedWords(self.process_sentence(line)).split()) > 0]

        # Build the people dictionary after the __sentences and removals are set
        self.__People = self.__buildPeopleDict()

    def __validate_file_path(self, file_path: str, description: str) -> str:
        """
    ...
    def process_sentence(self, sentence: str) -> str:
        ...
        if sentence == "":
            return sentence
        helperDict = hashDictForPunctions()  # Creating a hash dictionary to replace the values
    ...
    def __read_csv_to_list(self, file_path: str) -> list[str]:
        """
    ...
    def __buildPeopleDict(self) -> dict[str:list[str]]:
        ...
        if self.__PeopleInputPath is None:
            return {}
        peopleList = self.__readPepolFile(self.__PeopleInputPath)
        for person in peopleList:
            mainName = self.__removeUnwantedWords(self.process_sentence(person[0]))
            if mainName not in res:
                ...
                for nickName in nickNames:
                    if len(nickName) == 0:
                        pass
                    res[mainName].append(self.__removeUnwantedWords(self.process_sentence(nickName)))
            else:
                continue
        return res

    def __readPepolFile(self, filePath: str) -> list:
        """
    ...
    def writeTojsonFile(self, filePath: str | os.PathLike):
        """
            ...
            with open(filePath, 'w', encoding='utf-8') as f:
                data = {
                    "Question 1": {
                        ...
                        "Processed Names": self.__convert_to_nested_format(self.__People)
                    }
                }
    ...
    def __convert_to_nested_format(self, input_dict: dict[str:list[str]]) -> list[list[str]]:
        """
    ...
    def get_people(self) -> list[list[str]]:
        ...
        """
        return self.__convert_to_nested_format(self.__People)
...
if __name__ == '__main__':
    preprocessor = Preprocessor(QuestionNumber=1,
    ...
    print(preprocessor.getSentences())